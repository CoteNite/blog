...
当我们在使用RAG的时候会检索出来一系列的数据，这些数据的量极大，而我们的模型注意力和上下文均有限，这个时候我们就应该使用Rerank

所谓Rerank，就是将RAG检索出来的内容根据与我们的问题的相似度进行重排序，将相似度高的内容放置在上方，相似度低的内容放置在下方，这就让我们的模型获取到了更好的信息

## 大模型二次审查

虽然计算得到了相关的数据，但是没有交给大模型进行二次审查，将数据直接给到用户可能是有问题的，因此要通过大模型在对数据进行一轮的审核

## 混合检索

向量化检索是基于相似和语义化进行匹配，而有些场景则是对内容极其铭感，要求一字不差，这时候就需要使用关键字检索

这种检索方式在EsSearch中极其常见，就是我们的分词+倒排索引

如果同时使用多种检索方式，就称之为**混合检索**

这里很自然的又引出另一个问题，如果使用了混合检索，多种不同的检索得到的结果应该会有重复的文档，这个时候我们就应该将检索结果进行融合，然后在进行Rerank

## 排序算法  
### RRF  

公式：`score = sum(1/(k + rank))`，其中 `k`是一个常数（通常为60），`rank`是文档在单个列表中的排名

sum：两个集合

因为两张算法的评判标准不同，因此不能依靠分数来进行计算，而是各自的排名，因此在这个算法中，如果排名越靠后，而得出的分数会越低

**加权分数融合**  
虽然两种的评判标准不同，但是可以“归一化”将两边评判得分拉到同一层面，然后再设置权重：更偏向于向量/关键词进行计算