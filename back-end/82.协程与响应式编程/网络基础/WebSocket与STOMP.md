...
STOMP只是文本传输协议，不参与通信细节，我们可以将它视为一个简单的消息订阅模式，客户端和服务端都可以推送接受消息，而且它支持灵活的根据topic进行消息分发
...
当服务端和客户端建立连接后Spring就会维护一个topic和地址(习惯用`/`进行分割)的哈希表，这样用户就不用担心发送消息给谁的问题，直接操作topic即可
