...
## withContext与CoroutineScope.launch的区别

withContext和也是用于启动协程的，但是其与CoroutineScope.launch的最大区别在于他的运行的代码与其上下文是同步的关系，换句话说，就是使用withContext的代码会挂起原本的协程

其最大的意义在于在执行withContext的时候让平台线程被释放出来，去执行其他的任务，知道IO结束

这样对比，其实我们不难发现，withContext使用在协程下执行IO操作可以不阻塞原本的协程而使用的，但是CoroutineScope.launch则是启动一个协程，其作用是为了实现原Java中多线程的作用

另外，withContext还有一个很大的意义，就是逻辑顺序化，Kotlin协程的核心思想是希望异步代码能像正常运行的逻辑执行，也就是不需要写一大堆的回调函数，而withContext就能起到这个效果
...
## 同步的情况下使用suspend函数————runBlocking函数

有时候我们希望在同步的情况下使用suspend函数，这时可以使用Kotlin提供的runBlocking方法

其最大的意义是可以作为在suspend和普通函数之间的中转，进而将原本的平台线程运行时转换到协程运行时

我们直接翻看源码中对runBlocking的描述:

运行一个新的协程并中断地阻塞当前线程，直到其完成。它旨在将常规阻塞代码桥接到以挂起风格编写的库，以在main函数和测试中使用。

从挂起函数调用runBlocking是多余的。例如，下面的代码是不正确的：
...
在这里，如果fetchConfigurationData挂起，它不会释放loadConfiguration运行的线程，而是会阻塞，可能导致线程饥饿问题。

此构建器的默认CoroutineDispatcher是事件循环的内部实现，它处理此阻塞线程中的延续，直到此协程完成。请参阅CoroutineDispatcher了解kotlinx.coroutines提供的其他实现。

当CoroutineDispatcher在context中显式指定时，新协程将在指定调度程序的上下文中运行，同时当
前线程被阻塞。如果指定的调度程序是另一个runBlocking的事件循环，则此调用使用外部事件循环。

如果此阻塞线程被中断（请参阅Thread.interrupt ），则协程作业将被取消，并且此runBlocking调用将抛出InterruptedException 。
