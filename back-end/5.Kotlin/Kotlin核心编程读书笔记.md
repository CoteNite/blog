...
协变和逆变之所以在Kotlin中这么设计，实际上还是为了保证类型的安全性，而类型只有一种情况下是安全的————**当前类实例接受的是该类本身或子类的实例**

我们再来分别分析一下逆变和协变的设定：

- 协变只允许让泛型只能从当只读字段和方法出参/协变可以将子赋值给父：首先是只读字段，当将子赋值给父时，由于字段此时已经不可以修改，所以固定了此时我们拿到的一定是父亲的一个固定的子类，因此读出时也会是一个固定的子类，而当其作为出参时，由于是被当作父类使用，所以满足子替代父的关系