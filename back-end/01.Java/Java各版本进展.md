...
| 名称              | 对应  | 功能                                         | 使用场景          |
| --------------- | --- | ------------------------------------------ | ------------- |
| `fold`          | 多对一 | 将多个元素进行处理得到一个元素，通过多个元素间两两进行函数运算实现          | 平均值、总和、计数、最大值 |
| `scan`          | 一对一 | 消化元素、将其与内部状态整合，并计算  <br>累积中间值并产生新元素        | 前缀和、累积乘积、费式数列 |
| `mapConcurrent` | 一对一 | 对每个输入元素并行地呼叫函式，  <br>最多达到设定的上限             | 下载、读档、图片运算    |
| `windowFixed`   | 多对多 | 将元素依数量分组，在窗口满时向下传递                         | 批次处理、缓冲区读取    |
| `windowSliding` | 多对多 | 将元素依数量分组。每个后续窗口会将前  <br>一窗口的首元素删除后附加下个输入元素 | 移动平均、局部最大值    |