...
同时，在新的设计中，由于虚拟线程主动持有锁，因此就存在虚拟线程由于拿不到锁而被阻塞的情况，在这种情况虚拟线程会主动挂起，然后释放其对应的平台线程，这符合Project Loom对虚拟线程的设想
...
在JEP516中，不在将对象的引用固化为特定的GC内存地址，而是采用逻辑索引的方式与对象进行关联，逻辑索引的好处在于与GC无关，在这种情况下，我们可以直接将AOT使用在ZGC上

### [**JEP 517：HTTP/3 用于 HTTP 客户端 API**](https://openjdk.org/jeps/517)

在Java11中增加的HttpClient尚不能支持最新的Http/3协议，这一问题在JEP517中得到了解决，现在我们只需要简单的链式调用就可以为HttpClient提供http/3的支持
...
在过去的版本中，为了协调GC线程（进行回收）和应用线程（修改引用）之间的冲突，会使用写屏障来进行复杂的同步逻辑，但是这种复杂的同步逻辑在高并发，引用修改频繁的场景下会带来巨大的性能开销

JEP22中引入了双卡表机制

- 卡表：本身是一个字节数组，每一个字节代表了一快内存，当java代码修改了一个对象的引用字段