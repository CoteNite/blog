...
1. 扫描大量的JAR文件，并读取解析数千个类文件到内存中
2. 将解析后的类数据加载到类对象中并将其链接在一起，以便类可以使用彼此的API，这一过程涉及到字节码
...
现在你可以在程序运行前通过配置一些基本的AOT配置，然后进行训练运行，就可以生成AOT缓存，在有了AIT缓存的情况下，JVM 通常在程序运行第三步时执行的读取、解析、加载和链接工作被提前到了第二步，因此，程序在第三步启动速度更快，因为缓存中的类可以立即使用。
