...
这个收集器比较独特，又被称之为A No-Op Garbage Collector（无操作收集器），也就是说这个收集器其实不会进行回收操作，但是不进行回收的收集器又有什么意义呢？

实际上，收集器除了回收对象外，还承担着对堆进行管理，对创建的对象进行分配，与解释器编译器写作，监控子系统灯控能，因此"垃圾收集器"其实更应该叫做“自动内存管理子系统”

在JDK10后，RedHat希望能够隔离垃圾收集器与Java虚拟机的解释吗，编译，监控等功能的关联，因此提出统一的垃圾收集器接口，即JEP 304，Epsilon就是对这个接口可行性的实验

Epsilon也有自己的实际意义，可以设想这样一种场景，我们存在一个Java应用，应用运行并不长，大概率能保证在堆占用满前应用就会停止，在面对这样的应用时，我们其实不太需要垃圾回收这一功能，dan'lai'ji