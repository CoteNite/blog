...
这个算法的问题是会产生大量的内存复制开销，并且可以创建对象的空间缩小到了原来的一半。**不过这仍然是大多数JVM默认的回收算法**

根据三大假说，我们知道，一次回收中只会有一小部分的对象会仍然存活，因此就对半区复制进行了优化，即将新生代分为一个较大的Eden空间和两个较小的Survivor空间，创建对象时，会将对象创建在Eden和其中一个Survivor上，然后垃圾回收时，会将存活的对象放到另一个Survivor