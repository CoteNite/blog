...
在一个15wPV/日的文档系统中（PV——Page View，页面浏览次数），开发者发现在升级了服务器硬件后（32位系统升级至64位，并为系统进程分配了12g的内存）系统会出现不定时的长时间未响应。

在监控分析后，发现系统暂停的原因是频繁的发生Full GC，Full GC时间较长，且会暂停系统的用户进程。根据实际业务分析，是由于将过大的文档文件（n个g大小）输入到内存中，并转化为结构化的对象，导致产生了大量的大对象，根据JVM的处理逻辑，大对象会直接转入到老年代，故而导致老年代频繁被填满，进而造成频繁的Full GC

一个相对简答的解决方案是将老年代的内存改小，进而导致Full GC每次不用花费那么长的时间就能完成，进而解决暂停用户线程时间过长的问题。但是这样又会引回内存升级前的问题，也就是系统由于分配内存太小导致的卡顿

那么就考虑更换JVM，目前市面上主流针对延迟较长的问题的JVM有ZGC与Shenandoah，但是对于老系统（使用JDK5）更换JVM可能会产生其他的不可控问题，故而考虑优先级较低

进而考虑第三种方案，即使用JVM集群部署系统，即同时使用若干个JVM建立逻辑集群来利用硬件资源。做法是现在一台物理服务器上启动多个应用服务器进程，然后为每个服务器进程分配不同的端口号，在前端搭建负载均衡，通过反向代理的方式来分配请求。

在这种方式下，我们可以启动三个JVM构建集群，每个JVM分配4g内存（即升级前的两部大小），总使用内存仍为12g

当然，这种方案也并非是没有缺点，该方案会存在以下问题：
