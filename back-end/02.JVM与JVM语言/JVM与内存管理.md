...
由于主流的JVM采用的是可达性分析来判断一个对象是否应该被回收，因此我们就要先找到所有的GCRoots，这一过程也是要进行类似如同上文提到的Stop the world的暂停所有线程的过程，这是因为我们在枚举的这一刻不能允许存在指令对对象的引用进行修改，不然我们就无法保证回收的准确性

既然要寻找GCRoots，那么就要先知道哪些内存区域中存放有对象，HotSpot虚拟机中采用的是一种叫做OopMap的输一局结构，这个数据结构中存储了内存的哪块区域上有对象
