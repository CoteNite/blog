...
> 一个事务内一条数据的包含多次查询操作，结果查询的数据前后不一致

首先我们得理解一个事——幻读的问题其实是一个时间/原子性的问题，也就是我们认为/规定一个事务应该是`在一个时间点的状态下`完成的

但是由于事务包含多个操作，这些操作自然不可能同时完成，同是由于ACID还要求事务应该支持并发，这就导致了A事务的某个操作可能还在进行的过程中B事务就对表中A事务使用的数据进行了修改，这时由于我们希望事务的操作是`在一个时间点的状态下`完成的，这就发生了矛盾
